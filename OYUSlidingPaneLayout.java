 ...
 * </p>
 */
public class OYUSlidingPaneLayout extends ViewGroup {
    ...
    /**
     * Drawable used to draw the shadow between panes to support RTL (right to
     * left language).
     */
    ...
    public interface PanelSlideListener {
         ...
         * pane is visible.
         *
         * @param panel The child view that was slid to an open position,
    ...
    public OYUSlidingPaneLayout(Context context) {
        this(context, null);
    }

    public OYUSlidingPaneLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public OYUSlidingPaneLayout(Context context, AttributeSet attrs, int defStyle) {
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                // doug : 添加了 || canscroll... 这个分支，解决和viewpager之间的手势冲突
                if (adx > slop && ady > adx
    ...
    protected boolean drawChild(Canvas canvas, View child, long drawingTime) {
        ...
        // doug : 添加随拖拽缩小的效果
        float scale = 0.7f + 0.3f * mSlideOffset;
        if (child == mSlideableView) {
            float scales = 1f / scale * (0.6f + 0.4f * (1f - mSlideOffset));
            canvas.scale(scales, scales, child.getLeft(), child.getHeight() / 2);
        } else {
            canvas.scale(scale, scale, child.getLeft(), child.getHeight() / 2);
            ViewCompat.setAlpha(child, mSlideOffset);
        }
     ...
     * @deprecated Renamed to {@link #setShadowDrawableLeft(Drawable d)} to
     * support LTR (left to right language) and
     * {@link #setShadowDrawableRight(Drawable d)} to support RTL
     * (right to left language) during opening/closing.
     */
    @Deprecated
    ...
    /**
     * Set a drawable to use as a shadow cast by the left pane onto the right
     * pane during opening/closing to support right to left language.
     *
     * @param d drawable to use as a shadow
     */
    ...
    public void draw(Canvas c) {
        ...
        // doug : 由于是缩放处理，在此强制在container背后绘制阴影。
        if (shadowDrawable instanceof NinePatchDrawable) {
            NinePatchDrawable nph = (NinePatchDrawable) shadowDrawable;
            Rect padding = new Rect();
            nph.getPadding(padding);
            top = shadowView.getTop() - padding.top;
            bottom = shadowView.getBottom() + padding.bottom;
            left = shadowView.getLeft() - padding.left;
            right = shadowView.getRight() + padding.right;
        } else {
            int shadowWidth = shadowDrawable.getIntrinsicWidth();
            int shadowHeight = shadowDrawable.getIntrinsicHeight();
            top = shadowView.getTop() - shadowHeight;
            bottom = shadowView.getBottom() + shadowHeight;
            left = shadowView.getLeft() - shadowWidth;
            right = shadowView.getRight() + shadowWidth;
        }
    ...
    interface SlidingPanelLayoutImpl {
        void invalidateChildRegion(OYUSlidingPaneLayout parent, View child);
    }

    static class SlidingPanelLayoutImplBase implements SlidingPanelLayoutImpl {
        public void invalidateChildRegion(OYUSlidingPaneLayout parent, View child) {
    ...
    class AccessibilityDelegate extends AccessibilityDelegateCompat {
        ...
        public void onInitializeAccessibilityNodeInfo(View host, AccessibilityNodeInfoCompat info) {
            ...
            info.setClassName(OYUSlidingPaneLayout.class.getName());
            info.setSource(host);
        ...
        public void onInitializeAccessibilityEvent(View host, AccessibilityEvent event) {
            ...
            event.setClassName(OYUSlidingPaneLayout.class.getName());
        }
    ...
    private class DisableLayerRunnable implements Runnable {
        ...
        @Override
        public void run() {
            if (mChildView.getParent() == OYUSlidingPaneLayout.this) {
